...
WORD2VEC_EMBEDDINGS = '/home/slave1/桌面/vec/lime-experiments-master/our_dataset_embeddings.pickle'
class EmbeddingForest():
  ...
  inverse_vocabulary = None):
    ...
    self.embeddings = pickle.load(open(embedding_path, 'rb'))
    self.classifier = ensemble.RandomForestClassifier(n_estimators=3000, random_state=1, class_weight='balanced_subsample')
...
#    ret = []
#    for v in X:
#      words = [aa.inverse_vocabulary[x] for x in v.nonzero()[1]]
#      new = np.array([aa.embeddings[x] for x in words if x in aa.embeddings])
#      if new.shape[0] == 0:
#        new = np.zeros((1,300))
#      ret.append(np.mean(new, axis=0))
#    ret = np.array(ret)
#    return ret
#
#aa= transform_example(train_vectors)
#X=train_vectors
